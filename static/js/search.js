function searchbtn() {
    ...
    fetch('https://api.themoviedb.org/3/movie/top_rated?language=en-US&page=1', options)
        ...
        .then(data => {
            ...
            // 'filtered_title_array'가 비어있을 경우, "해당 영화를 찾을 수 없습니다!"라는 알림이 표시됩니다.
            if (filtered_title_array.length == 0) {
                alert("해당 영화를 찾을 수 없습니다!")
                return location.reload()
            }

            // 검색어가 입력되지 않았을 경우, "영화의 제목을 입력해주세요!"이라는 알림이 표시됩니다.
            if (search_string == false) {
                alert("영화의 제목을 입력해주세요!")
                return location.reload()
            }
            ...
            rows.forEach((movie) => {
                ...
                let temp = `<div class="movie-card" onclick = 'alert("${id}")'>
